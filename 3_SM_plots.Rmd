...
water_mask_directory <- './data/masks/' 
...
Data on bulk discharge metrics and discharge variability computed from previous scripts.
...
```{r planform_TR_hist, eval = F}
dfj_planform <- df_joined %>% 
  # mutate(nchan = ifelse(Channel.Count.Index.... == 1, 'Single-thread', 'Multi-thread'))
  mutate(nchan = ifelse(Type == 'Me', 'Single-thread', 'Multi-thread'))

ggplot(dfj_planform, aes(x = TR, fill = nchan)) + 
  geom_histogram(colour = 'lightgrey', alpha = 0.6, position = 'identity') + 
...
```{r Qmon_an_v_TR, fig.height=3, fig.width=6, dpi=300, eval = F}
# plotting params
lposy <- c(165, 151, 137)
xadj <- -0.2
...
```{r width_plots, fig.height=3, fig.width=6, dpi=300, eval = F}
# plotting params
...
```{r exp_alt, fig.height=6, fig.width=6, dpi=300, eval = F}
# plotting params
lposy <- c(165, 151, 137)
xadj <- -0.2
...
cor(rf_dat %>% select(-TR, -Channel.Count.Index....)) # test correlation between predictors
...
rownames(outtab) <- c('Richards-Baker Index, $RBI$ (-)', # rename model variables *BEFORE SORTING*
                      'Average Discharge Variability Index, $DVI_a$ (-)', 
...
sub1 <- rf_dat_planform %>% filter(Type == 'Me') %>% 
  select(-Type)
sub2 <- rf_dat_planform %>% filter(Type != 'Me')%>% 
  select(-Type)
subs <- list(sub1, sub2)
...
```{r soi_imports, eval = F}
# ---sites of interest----

# Comment out as needed to make a comparison pair for each planform type.
...
```{r conceptual_fig_base_meandering, fig.width = 7, fig.height = 8, dpi=300, eval = F}
par(oma = c(0, 0, 0 , 0),
    mai = c(0.5, 0.5, 0, 0),
    mgp = c(1.5, 0.5, 0))
...
# Table of water mask history for each site. Classify each site by 'mobility type':

```{r water_mask_history, eval = T, fig.width=7, fig.height=8, dpi=300}
require(terra)

# Classification of each site by "mobility type":
mob_class <- read.csv('./data/SM_data/mobility_class_final.csv')
...
for(i in 1:nrow(df_joined)){ # comment if debugging
# for(i in 43:48){ # uncomment if debugging
  
  # name site of interest
  soi = df_joined$River[i]
  
  mob_class_i = mob_class$mobil_type_final[mob_class$River == soi]
  
  #------ import water masks ------
  
  rastdir <- paste0(water_mask_directory, '/SingleRivers/RiverData/', 
                    df_joined$Type[df_joined$River == soi], '/', soi)
  tifs <- list.files(rastdir, recursive = T, full.names = T, pattern = '.tif')
  tifs_aux <- list.files(rastdir, pattern = '.tif.aux', full.names = T, recursive = T)
  tifs <- base::setdiff(tifs, tifs_aux) # get rid of aux files from list of files to import
  st <- rast(tifs)
  ...
  if(soi == 'Tanana_NearHardingLake' | soi == 'Tista_AndersonBr'){ 
    tvec_yr <- as.numeric(stringr::str_sub(tifs, -24, -21) )
  }
  
  # assign timestamps to raster stack
  time(st) <- tvec_yr 
  ...
  # special cases - when one of standard years is missing
  if(soi == 'Tanana_NearHardingLake'){ 
    yrs <- c(1991, 2001, 2011, 2021)
  } else if(soi == 'Amur_Khabarovsk') {
    yrs <- c(1990, 2000, 2011, 2020)
  } else if(soi == 'Katun_Srostki') {
    yrs <- c(1994, 2001, 2011, 2021)
  } else if(soi == 'Kokcha_Khojaghar') {
    yrs <- c(1990, 2000, 2010, 2019)
  } else if(soi == 'Lena_Tabaga') {
    yrs <- c(2000, 2010, 2020)
  } else if(soi == 'Ob_Prokhorkino') {
    yrs <- c(1986, 2000, 2010, 2020)
  } else if(soi == 'Parana_Corrientes') {
    yrs <- c(1990, 2000, 2009, 2020)
  } else if(soi == 'Solimoes_Tabatinga') {
    yrs <- c(1991, 2000, 2010, 2020)
  } else if(soi == 'Taku_NearTulsequa') {
    yrs <- c(1991, 2000, 2010, 2020)
  } else if(soi == 'Tista_AndersonBr') {
    yrs <- c(1993, 2000, 2009, 2019)
  } else if(soi == 'Tista_Kaunia') {
    yrs <- c(1990, 2000, 2009, 2020)
  } else if(soi == 'Yukon_NearStevensVillage') {
    yrs <- c(1986, 2000, 2009, 2020)
  } else { 
  ...
  plot(st[[time(st) == yrs[1]]],
       ...
       buffer = F,
       clip = T,
       main = paste0(soi, ': ', mob_class_i)) # first raster
  pu <- par('usr') # save axis lims to object
  
  # scale bar
  # sites requiring shifts to avoid covering data
  if(soi == 'Brahmaputra_Bahadurabad' | soi == 'Irrawaddy_Sagaing' | soi == 'SaoFrancisco_BomJesusDaLapa' | soi == 'Tisza_Vylok'){ 
      sbar(5000, xy="bottomleft", divs=2, cex=1, ticks=TRUE, label = ('5 km')) 
  } else if(soi == 'Brahmaputra_Yangcun' ){
    sbar(5000, xy="topright", divs=2, cex=1, ticks=TRUE, label = ('5 km')) 
  } else if(soi == 'Lena_Tabaga' | soi == 'Mamore_PuertoSiles' | soi == 'Parana_Corrientes'){
    sbar(5000, xy="bottomright", divs=2, cex=1, ticks=TRUE, label = ('5 km')) 
  } else { 
    
    # general condition
      sbar(5000, xy="bottom", divs=2, cex=1, ticks=TRUE, label = ('5 km'))
  }
  
  
  for(j in 2:length(yrs)){ # superimpose next layers 
    plot(st[[time(st) == yrs[j]]], 
         col = c(NA, colvec[si-1 + j]),
         axes = F,
         legend = F,
         mar = c(1, 0, 2, 0),
...
# Table of mobility types

## Option 1: group by mobility class

```{r mob_class_tab1}

# join data for each site
mob_class <- read.csv('./data/SM_data/mobility_class_final.csv') %>% 
  select(River, mobil_type_final) %>% 
  full_join(., df_joined) %>% 
  mutate(plan_simple = ifelse(Type == 'LSW' | Type == 'HSW', 'W', Type)) %>% 
  group_by(mobil_type_final)
  
mob_sum <- mob_class %>% 
  summarise(n = n(),
            Br = sum(plan_simple == 'B'),
            Me = sum(plan_simple == 'Me'),
            Wa = sum(plan_simple == 'W'),
            TR_med = round(median(TR), 0),
            RBI_med = signif(median(RBI), 2))

# rename cols:
colnames(mob_sum) <- c('Style', 'N.', 'Br', 'Me', 'Wa', 
                       '$T_{R50}$',
                       '$RBI_{50}$')
...
## Option 2: group by planform and TR
...
mob_class <- read.csv('./data/SM_data/mobility_class_final.csv') %>% 
  ...
  full_join(., df_joined) %>% 
  mutate(plan_simple = ifelse(Type == 'LSW' | Type == 'HSW', 'W', Type)) %>% 
  mutate(thread = ifelse(Type == 'Me', 'Single', 'Multi')) %>% 
  group_by(thread) %>% 
  mutate(subclass = cut_number(TR, 2)) %>% 
  group_by(subclass)

# divide into two equal bins in each group.
mob_sum <- mob_class %>% 
  summarise(thread = unique(thread),
            TR = round(median(TR), 0), 
            RBI = signif(median(RBI), 2),
            DVIa = signif(median(DVIa), 2),
            DVIc = signif(median(DVIc), 2),
            BM = sum(mobil_type_final == 'BM'),
            BMC = sum(mobil_type_final == 'BMC'),
            BMA = sum(mobil_type_final == 'BMA'),
            ABM = sum(mobil_type_final == 'ABM'),
            Me = sum(plan_simple == 'Me'),
            Wa = sum(plan_simple == 'W'),
            Br = sum(plan_simple == 'B')) %>% 
  mutate(subclass = c('low', 'high', 'low', 'high')) %>% 
  relocate(thread, .before = subclass)


# rename cols:
colnames(mob_sum) <- c('Threads', '$T_R$', '$T_{R50}$', '$RBI_{50}$',
                       '$DVI_{a50}$', '$DVI_{c50}$',
                       'BM', 'BMC', 'BMA', 'ABM', 'Me', 'Wa', 'Br')
...
# data prep - manually measured dist to nearest dam upstream at each site...
dams <- st_read('./data/mobility/area_based/polygons/areal_all_w_dams.shp') %>% 
  rename(River = site) %>% 
  st_drop_geometry() %>% 
  select(-Field) %>% 
  mutate(dam_dist = as.numeric(dam_dist))

df_joined <- df_joined %>% 
  left_join(dams)

# omit dams < 10 km upstream
dams_gte_10_km <- df_joined %>% 
  filter(dam_dist >= 10 | is.na(dam_dist))

# omit dams < 50 km upstream
dams_gte_50_km <- df_joined %>% 
  filter(dam_dist >= 50 | is.na(dam_dist))

# omit dams < 100 km upstream
dams_gte_100_km <- df_joined %>% 
  filter(dam_dist >= 100 | is.na(dam_dist))

# omit dams at any dist. on mainstem
dams_trib_only <- df_joined %>% 
  filter(Dam_notes != 'on mainstem' | is.na(Dam_notes))

```

## Figure 3, omitting sites with a dam less than _n_ km upstream

Re-write as func or loop if time.
...
```{r Qvar_v_TR_10km, fig.height=2.5, fig.width=6, dpi=300, eval = F}
# subset data
sub1 <- dams_gte_10_km %>% filter(Type == 'Me')
sub2 <- dams_gte_10_km %>% filter(Type != 'Me')
subs <- list(sub1, sub2)
...
# plot
for(i in 1:length(Qvars)){
  bvar <- colnames(subset(dams_gte_10_km, select = Qvars))[i]
  
  # set plot axes
  par(xpd = NA)
  plot(range(dams_gte_10_km[bvar], na.rm = T), range(dams_gte_10_km$TR),
       xlab = xlabs[i], 
       ylab = bquote('Reworking timescale,' ~ italic('T'[R]) ~ ' (yr)'), 
       type = 'n', 
       log = '')  # if log-transform needed
  ...
  # fit exponential model to full dataset:
  x = dams_gte_10_km[bvar] %>% pull() %>% replace(., .<=0.0, NA) # substitute zeros or negatives with NA
  y = dams_gte_10_km$TR
...
```{r Qvar_v_TR_50km, fig.height=2.5, fig.width=6, dpi=300, eval = F}
# subset data
sub1 <- dams_gte_50_km %>% filter(Type == 'Me')
sub2 <- dams_gte_50_km %>% filter(Type != 'Me')
subs <- list(sub1, sub2)
...
# plot
for(i in 1:length(Qvars)){
  bvar <- colnames(subset(dams_gte_50_km, select = Qvars))[i]
  
  # set plot axes
  par(xpd = NA)
  plot(range(dams_gte_50_km[bvar], na.rm = T), range(dams_gte_50_km$TR),
       xlab = xlabs[i], 
       ylab = bquote('Reworking timescale,' ~ italic('T'[R]) ~ ' (yr)'), 
       type = 'n', 
       log = '')  # if log-transform needed
  ...
  # fit exponential model to full dataset:
  x = dams_gte_50_km[bvar] %>% pull() %>% replace(., .<=0.0, NA) # substitute zeros or negatives with NA
  y = dams_gte_50_km$TR
...
```{r Qvar_v_TR_100km, fig.height=2.5, fig.width=6, dpi=300, eval = F}

# subset data
sub1 <- dams_gte_100_km %>% filter(Type == 'Me')
sub2 <- dams_gte_100_km %>% filter(Type != 'Me')
subs <- list(sub1, sub2)
...
# plot
for(i in 1:length(Qvars)){
  bvar <- colnames(subset(dams_gte_100_km, select = Qvars))[i]
  
  # set plot axes
  par(xpd = NA)
  plot(range(dams_gte_100_km[bvar], na.rm = T), range(dams_gte_100_km$TR),
       xlab = xlabs[i], 
       ylab = bquote('Reworking timescale,' ~ italic('T'[R]) ~ ' (yr)'), 
       type = 'n', 
       log = '')  # if log-transform needed
  ...
  # fit exponential model to full dataset:
  x = dams_gte_100_km[bvar] %>% pull() %>% replace(., .<=0.0, NA) # substitute zeros or negatives with NA
  y = dams_gte_100_km$TR
...
## Figure 3, omitting sites with a dam on the main stem


```{r Qvar_v_TR_tribonly, fig.height=2.5, fig.width=6, dpi=300, eval = F}

# subset data
sub1 <- dams_trib_only %>% filter(Type == 'Me')
sub2 <- dams_trib_only %>% filter(Type != 'Me')
subs <- list(sub1, sub2)
...
# plot
for(i in 1:length(Qvars)){
  bvar <- colnames(subset(dams_trib_only, select = Qvars))[i]
  
  # set plot axes
  par(xpd = NA)
  plot(range(dams_trib_only[bvar], na.rm = T), range(dams_trib_only$TR),
       xlab = xlabs[i], 
       ylab = bquote('Reworking timescale,' ~ italic('T'[R]) ~ ' (yr)'), 
       type = 'n', 
       log = '')  # if log-transform needed
  ...
  # fit exponential model to full dataset:
  x = dams_trib_only[bvar] %>% pull() %>% replace(., .<=0.0, NA) # substitute zeros or negatives with NA
  y = dams_trib_only$TR
